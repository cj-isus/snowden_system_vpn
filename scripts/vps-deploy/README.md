...
Самый дешёвый вариант: лендинг и файлы отдаются прямо с сервера `89.125.1.217` на порту `8090`.
Работает из РФ **без VPN**, пока конкретный IP не попал в реестр (свежий сервер — нет).
Плюс: весь конфиг уже указывает на `http://89.125.1.217:8090`.
...
│   ├── index.html           # лендинг (кнопки → http://89.125.1.217:8090/...)
...
## Как запустить на сервере (нужен SSH/root к 89.125.1.217)

```bash
# залить папку на сервер (например в /root/vps-deploy)
scp -r vps-deploy root@89.125.1.217:/root/
# на сервере
...
После этого лендинг доступен: `http://89.125.1.217:8090`
...
```
SNOWDEN_FILE_URL=http://89.125.1.217:8090
```
